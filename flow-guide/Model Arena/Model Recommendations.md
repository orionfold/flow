...
models:
  - id: mlx-community/gemma-3-text-4b-it-4bit
    ...
    note: Composes 8 of 8 on Flow's agency baseline in one turn; 91 tokens a second, 1.6 s to the first word (calculated); reads 1096 tokens a second; the Mac stayed nominal while measured.
  - id: mlx-community/Mistral-7B-Instruct-v0.2-4bit
    ...
    measuredTier: 36
    qualitySource: LMArena lmarena-ai/leaderboard-dataset text_style_control/latest, published 2026-09-02
    note: Composes 8 of 8 on Flow's agency baseline in one turn; 57 tokens a second, 2.8 s to the first word (calculated); reads 629 tokens a second; the Mac stayed nominal while measured.
  - id: mlx-community/Llama-3.1-8B-Instruct-4bit
    ...
    measuredTier: 36
    qualitySource: LMArena lmarena-ai/leaderboard-dataset text_style_control/latest, published 2026-09-02
    note: Composes 8 of 8 on Flow's agency baseline in one turn; 55 tokens a second, 2.8 s to the first word (calculated); reads 617 tokens a second; the Mac stayed nominal while measured.
  - id: mlx-community/gemma-4-e4b-it-4bit
    ...
    note: Composes 8 of 8 on Flow's agency baseline in one turn; 72 tokens a second, 1.1 s to the first word (calculated); reads 1631 tokens a second; the Mac stayed nominal while measured.
  - id: mlx-community/granite-4.1-8b-4bit
    ...
    promptTokensPerSecond: 492.6
    worstThermalState: nominal
    nightBaselineComposed: 1
    nightBaselineProbeCount: 3
    measuredTier: 36
    qualitySource: LMArena lmarena-ai/leaderboard-dataset text_style_control/latest, published 2026-09-02
    note: Loads and runs on Flow Runtime; the agency baseline has not been run on it; 45 tokens a second, 3.5 s to the first word (calculated); reads 493 tokens a second; the Mac stayed nominal while measured; below the day speed floor.
  - id: mlx-community/gemma-3n-E4B-it-lm-4bit
    ...
    promptTokensPerSecond: 761.5
    worstThermalState: nominal
    nightBaselineComposed: 0
    nightBaselineProbeCount: 3
    measuredTier: 36
    qualitySource: LMArena lmarena-ai/leaderboard-dataset text_style_control/latest, published 2026-09-02
    note: Loads and runs on Flow Runtime; the agency baseline has not been run on it; 61 tokens a second, 2.3 s to the first word (calculated); reads 762 tokens a second; the Mac stayed nominal while measured.
  - id: mlx-community/gemma-3-text-12b-it-4bit
    ...
    measuredTier: 36
    qualitySource: LMArena lmarena-ai/leaderboard-dataset text_style_control/latest, published 2026-09-02
    note: Composes 1 of 8 on Flow's agency baseline in one turn, so not yet a pick; 34 tokens a second, 5.1 s to the first word (calculated); reads 338 tokens a second; the Mac stayed nominal while measured; below the day speed floor.
  - id: mlx-community/gemma-4-26b-a4b-it-4bit
    ...
    workingSetBytes: 18125123752
    evidence: measured
    evidenceSource: signed receipt 6e9d4b387f549419d6a688dbbe1e1b9222633cef2b3004e6b76807fe7df7f23e
    tokensPerSecond: 73.3
    timeToFirstTokenSeconds: 1.9
    promptTokensPerSecond: 928.9
    worstThermalState: nominal
    baselineComposed: 8
    baselineProbeCount: 8
    ...
    measuredTier: 36
    qualitySource: LMArena lmarena-ai/leaderboard-dataset text_style_control/latest, published 2026-09-02
    note: Composes 8 of 8 on Flow's agency baseline with its own lookups; 73 tokens a second, 1.9 s to the first word (calculated); reads 929 tokens a second; the Mac stayed nominal while measured.
  - id: mlx-community/Qwen3.8-27B-4bit
    ...
    workingSetBytes: 19060257392
    evidence: measured
    evidenceSource: signed receipt 6e9d4b387f549419d6a688dbbe1e1b9222633cef2b3004e6b76807fe7df7f23e
    tokensPerSecond: 16.8
    timeToFirstTokenSeconds: 11.4
    promptTokensPerSecond: 151.4
    worstThermalState: nominal
    baselineComposed: 8
    baselineProbeCount: 8
    nightBaselineComposed: 8
    ...
    measuredTier: 36
    qualitySource: LMArena lmarena-ai/leaderboard-dataset text_style_control/latest, published 2026-09-02
    note: Composes 8 of 8 on Flow's agency baseline with its own lookups; 17 tokens a second, 11.4 s to the first word (calculated); reads 151 tokens a second; the Mac stayed nominal while measured.
  - id: lmstudio-community/Qwen3.8-27B-MLX-5bit
    displayName: Qwen 3.8 27B 5-bit
    format: mlx
    weightBytes: 19443171157
    agencyBaseline: flow-agency-v2
    class: night
    ...
    servedWindow: 8192
    kvBytesPerToken: 65536
    workingSetBytes: 22366976952
    evidence: measured
    evidenceSource: signed receipt 6e9d4b387f549419d6a688dbbe1e1b9222633cef2b3004e6b76807fe7df7f23e
    tokensPerSecond: 13.8
    timeToFirstTokenSeconds: 12.2
    promptTokensPerSecond: 141.3
    worstThermalState: nominal
    baselineComposed: 8
    baselineProbeCount: 8
    nightBaselineComposed: 8
    ...
    measuredTier: 36
    qualitySource: LMArena lmarena-ai/leaderboard-dataset text_style_control/latest, published 2026-09-02
    note: Composes 8 of 8 on Flow's agency baseline with its own lookups; 14 tokens a second, 12.2 s to the first word (calculated); reads 141 tokens a second; the Mac stayed nominal while measured.
---
...
You can have all three. Flow picks between them per task, and always tells you which it used.
...
**Format and dialect.** The Format column describes this build's engine capability. Both owned engines constrain supported output shapes when a run requests them; a run without a requested shape remains unconstrained. The measurements and baseline results below keep their original runtime and envelope. Earlier prompt-only MLX runs have not been remeasured by this runtime update; their recorded dialect remains in [[Model Evidence]]. Flow still validates every result before using it.

### 16 GB

| Class | Model | Format | Download | Working set | Speed | Agency baseline | Evidence |
| --- | --- | --- | ---: | ---: | --- | --- | --- |
| day (candidate for both) | **Gemma 3 4B IT (language-model-only export) 4-bit** (Flow's pick) | MLX, grammar dialect | 2.6 GB | 4.6 GB | 91 tok/s, 1.6 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
| day (candidate for both) | Mistral 7B Instruct v0.2 4-bit | MLX, grammar dialect | 4.3 GB | 6.4 GB | 57 tok/s, 2.8 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
| night (candidate for both) | Gemma 3 4B IT (language-model-only export) 4-bit | MLX, grammar dialect | 2.6 GB | 4.6 GB | 91 tok/s, 1.6 s to first word (calculated) | Not yet (0 of 3) | measured on 36 GB; projected here |
| night (candidate for both) | Mistral 7B Instruct v0.2 4-bit | MLX, grammar dialect | 4.3 GB | 6.4 GB | 57 tok/s, 2.8 s to first word (calculated) | Not yet (0 of 3) | measured on 36 GB; projected here |
...
| Class | Model | Format | Download | Working set | Speed | Agency baseline | Evidence |
| --- | --- | --- | ---: | ---: | --- | --- | --- |
| day (candidate for both) | Gemma 3 4B IT (language-model-only export) 4-bit | MLX, grammar dialect | 2.6 GB | 4.6 GB | 91 tok/s, 1.6 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
| day (candidate for both) | Mistral 7B Instruct v0.2 4-bit | MLX, grammar dialect | 4.3 GB | 6.4 GB | 57 tok/s, 2.8 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
| day | Llama 3.1 8B Instruct 4-bit | MLX, grammar dialect | 4.5 GB | 6.5 GB | 55 tok/s, 2.8 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
| day | **Gemma 4 E4B IT 4-bit** (Flow's pick) | MLX, grammar dialect | 5.2 GB | 6.6 GB | 72 tok/s, 1.1 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
| day | Granite 4.1 8B 4-bit | MLX, grammar dialect | 5.2 GB | 7.0 GB | 45 tok/s, 3.5 s to first word (calculated) (below the day speed floor) | Not run | measured on 36 GB; projected here |
| day | Gemma 3n E4B IT (language-model-only export) 4-bit | MLX, grammar dialect | 3.9 GB | 7.2 GB | 61 tok/s, 2.3 s to first word (calculated) | Not run | measured on 36 GB; projected here |
| night (candidate for both) | Gemma 3 4B IT (language-model-only export) 4-bit | MLX, grammar dialect | 2.6 GB | 4.6 GB | 91 tok/s, 1.6 s to first word (calculated) | Not yet (0 of 3) | measured on 36 GB; projected here |
| night (candidate for both) | Mistral 7B Instruct v0.2 4-bit | MLX, grammar dialect | 4.3 GB | 6.4 GB | 57 tok/s, 2.8 s to first word (calculated) | Not yet (0 of 3) | measured on 36 GB; projected here |
| night | *No pick yet* | | | | | No night model has earned the pick yet; Mistral 7B Instruct v0.2 4-bit is the leading candidate at 0 of 3 | |
...
| day | Granite 4.1 8B 4-bit | MLX, grammar dialect | 5.2 GB | 7.0 GB | 45 tok/s, 3.5 s to first word (calculated) (below the day speed floor) | Not run | measured on 36 GB; projected here |
| day | Gemma 3n E4B IT (language-model-only export) 4-bit | MLX, grammar dialect | 3.9 GB | 7.2 GB | 61 tok/s, 2.3 s to first word (calculated) | Not run | measured on 36 GB; projected here |
| day | Gemma 3 12B IT (language-model-only export) 4-bit | MLX, grammar dialect | 7.2 GB | 10.4 GB | 34 tok/s, 5.1 s to first word (calculated) (below the day speed floor) | Not yet (1 of 8) | measured on 36 GB; projected here |
| night (candidate for both) | Gemma 3 4B IT (language-model-only export) 4-bit | MLX, grammar dialect | 2.6 GB | 4.6 GB | 91 tok/s, 1.6 s to first word (calculated) | Not yet (0 of 3) | measured on 36 GB; projected here |
| night (candidate for both) | Mistral 7B Instruct v0.2 4-bit | MLX, grammar dialect | 4.3 GB | 6.4 GB | 57 tok/s, 2.8 s to first word (calculated) | Not yet (0 of 3) | measured on 36 GB; projected here |
| night | Gemma 4 26B A4B IT 4-bit | MLX, grammar dialect | 15.4 GB | 18.1 GB | 73 tok/s, 1.9 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
| night | **Qwen 3.8 27B 4-bit** (Flow's pick) | MLX, grammar dialect | 16.1 GB | 19.1 GB | 17 tok/s, 11.4 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
...
| night (candidate for both) | Mistral 7B Instruct v0.2 4-bit | MLX, grammar dialect | 4.3 GB | 6.4 GB | 57 tok/s, 2.8 s to first word (calculated) | Not yet (0 of 3) | measured on 36 GB; projected here |
| night | Gemma 4 26B A4B IT 4-bit | MLX, grammar dialect | 15.4 GB | 18.1 GB | 73 tok/s, 1.9 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
| night | Qwen 3.8 27B 4-bit | MLX, grammar dialect | 16.1 GB | 19.1 GB | 17 tok/s, 11.4 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
| night | **Qwen 3.8 27B 5-bit** (Flow's pick) | MLX, grammar dialect | 19.4 GB | 22.4 GB | 14 tok/s, 12.2 s to first word (calculated) | Meets it (8 of 8) | measured on 36 GB; projected here |
...
Every list row, every measurement cell and every baseline run behind these tables, probe by probe, is in [[Model Evidence]]. For repeatable Flow Runtime measurements on named Mac hardware, see [[Model Comparisons]]. That page retains historical runtime measurements. They provide context for the current picks; a selection changes through the qualified Flow-job comparison described here.
